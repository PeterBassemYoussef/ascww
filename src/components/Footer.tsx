                ...
                <div className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
                    <div className="h-full">
                        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-[#f1f5f9d9] p-3 shadow-[0_14px_35px_rgba(2,6,23,0.35)]">
                            <div className="mb-3 flex items-center justify-between">
                                <h3 className="text-base font-bold text-slate-900">{t('مقر الشركة', 'Company Headquarters')}</h3>
                                <span className="rounded-full bg-[#0a3555]/80 px-2.5 py-1 text-[11px] font-bold text-slate-100">{t('أسيوط', 'Assiut')}</span>
                            </div>
                            <iframe
                                title={t('موقع شركة مياه الشرب والصرف الصحي بأسيوط والوادي الجديد', 'Assiut and New Valley Water and Wastewater Company location')}
                    ...
                    <div className="h-full px-1 py-1">
                        ...
                        <div className="mt-5 flex flex-wrap items-center gap-2">
                            ...
                            <a href="https://api.whatsapp.com/send?phone=201281565653" target="_blank" rel="noopener noreferrer" aria-label={t('واتساب', 'WhatsApp')} className="social-icon social-icon--whatsapp">
                            ...
                            <a href="https://youtube.com/channel/UC73LZeR5Yr5TE7fsTzvZSVw" target="_blank" rel="noopener noreferrer" aria-label={t('يوتيوب', 'YouTube')} className="social-icon social-icon--youtube">
                    ...
                    <div className="h-full px-1 py-1">
                        <h3 className="mb-4 text-base font-bold text-white">{t('روابط تهمك', 'Useful Links')}</h3>
                        <div className="space-y-2 text-sm">
                            <a className="block transition hover:text-white" href="http://bills.ascww.com.eg/Inqeury.aspx" target="_blank" rel="noopener noreferrer">{t('استعلم عن فاتورتك', 'Check your bill')}</a>
                            <Link className="block transition hover:text-white" to={ROUTES.callCenter}>{t('خدمة العملاء', 'Customer service')}</Link>
                            <Link className="block transition hover:text-white" to={ROUTES.projectsArchive}>{t('أرشيف المشروعات', 'Projects archive')}</Link>
