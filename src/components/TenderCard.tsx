...
function TenderCard({ tender }: TenderCardProps) {
    ...
    const expiryDate = formatDate(tender.expiration_date);
    const createdDate = formatDate(tender.created_at || tender.updated_at);

    return (
        <article className="tender-card group flex w-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md md:flex-row" dir={isEnglish ? 'ltr' : 'rtl'}>
                ...
                <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
                    ...
                    {expiryDate && (
                        <span className="rounded-full bg-amber-50 px-2.5 py-1 font-bold text-amber-700">
                            {t('آخر موعد:', 'Deadline:')} {expiryDate}
                        </span>
                    )}
                    {!expiryDate && createdDate && (
                        <span className="rounded-full bg-slate-100 px-2.5 py-1 font-bold text-slate-600">
                            {isEnglish ? `Published: ${createdDate}` : createdDate}
                        </span>
                    )}
                </div>

                <h3 className={`mb-2 line-clamp-2 text-lg font-extrabold leading-8 text-slate-900 ${textAlignmentClass}`}>
                    <Link to={detailsPath} className="transition-colors hover:text-[#0a3555]">
                        {tender.title || t('مناقصة بدون عنوان', 'Untitled tender')}
                    </Link>
                </h3>

                <p className={`mb-5 line-clamp-3 flex-1 text-sm leading-7 text-slate-600 ${textAlignmentClass}`}>
                    {descriptionText || t('لا توجد تفاصيل مختصرة متاحة.', 'No short details are available.')}
                </p>

                <div className={footerClass}>
                    <Link
                        to={detailsPath}
                        className="group inline-flex items-center gap-1 text-sm font-bold text-[#0a3555] transition-colors hover:text-[#082b47]"
                    >
