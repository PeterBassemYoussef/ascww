...
function LabOfCompanyPage() {
  ...
  useEffect(() => {
    ...
    const loadImages = async () => {
      try {
        const sources = await fetchGallerySources('labs_dep', controller.signal);
        ...
        <div className="container mx-auto max-w-7xl px-4 py-8 md:py-10">
          <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_20px_55px_rgba(2,6,23,0.08)]">
            <div className={`border-b border-[#d7b05a]/35 bg-white px-6 py-7 text-[#0a3555] sm:px-8`}>
              <h1 className="text-3xl font-black leading-tight text-[#0a3555] sm:text-4xl">{t('معامل الشركه', 'Company Laboratories')}</h1>
            </div>
